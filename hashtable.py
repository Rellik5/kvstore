...
if __name__ == "__main__":
    ...
    ht.set("name", "roman")
    ht.set("age", "21")
    print(ht.get("name"))     # roman
    print(ht.get("age"))      # 27
    print(ht.get("missing"))  # None

    ht.set("name", "changed") # overwrite the same key
    print(ht.get("name"))     # changed  <- last write wins
    print(ht.delete("age"))   # True  <- age existed, so it's removed
    print(ht.get("age"))      # None  <- confirms it's gone
    print(ht.delete("age"))   # False <- already gone, nothing to delete